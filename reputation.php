...
if($mybb->input['action'] == "add" || $mybb->input['action'] == "do_add")
{
	...
	if($mybb->usergroup['cangivereputations'] != 1)
	{
		...
		{
			eval($templates->render("reputation_add_error_nomodal", "error", false, 1, 0));
		}
		else
		{
			eval($templates->render("reputation_add_error", "error", false, 1, 0));
		}
		echo $error;
		exit;
	}
...
if($mybb->input['action'] == "do_add" && $mybb->request_method == "post")
{
	...
	if(!empty($mybb->input['delete']))
	{
		...
		eval($templates->render("reputation_deleted", "error", false, 1, 0));
		echo $error;
		exit;
	}
	...
	eval($templates->render("reputation_added", "reputation", false, 1, 0));
	echo $reputation;
	exit;
}
...
if($mybb->input['action'] == "add")
{
	...
	if(!empty($existing_reputation['uid']) || !empty($existing_post_reputation['uid']))
	{
		...
		eval($templates->render("reputation_add_delete", "delete_button"));
	}
	...
	if($mybb->settings['negrep'] || $mybb->settings['neurep'] || $mybb->settings['posrep'])
	{
		...
		if($mybb->settings['neurep'])
		{
			...
			eval($templates->render("reputation_add_neutral", "neutral_power"));
		}
		...
		for($value = 1; $value <= $reputationpower; ++$value)
		{
			if($mybb->settings['posrep'])
			{
				...
				eval($templates->render("reputation_add_positive", "positive_power"));
			}
			...
			if($mybb->settings['negrep'])
			{
				...
				$neg_value = "-{$value}";
				eval($templates->render("reputation_add_negative", "negative_power", true));
			}
		}
		...
		eval($templates->render("reputation_add", "reputation_add", false, 1, 0));
	}
	else
	{
...
if(!$mybb->input['action'])
{
	...
	{
		eval($templates->render("reputation_addlink", "add_reputation"));
	}
	else
	{
		$add_reputation = '';
	...
	foreach($reputation_cache as $reputation_vote)
	{
		...
		{
			eval($templates->render("reputation_vote_delete", "delete_link"));
		}

		$report_link = '';
		if($mybb->user['uid'] != 0)
		{
			eval($templates->render("reputation_vote_report", "report_link"));
		}
		...
		eval($templates->render("reputation_vote", "reputation_votes", true));
	}

	// If we don't have any reputations display a nice message.
	if(!$reputation_votes)
	{
		eval($templates->render("reputation_no_votes", "reputation_votes"));
	}

	$plugins->run_hooks("reputation_end");
	eval($templates->render("reputation", "reputation"));
