...
if($mybb->input['action'] == "add" || $mybb->input['action'] == "do_add")
{
	...
	if(!empty($message))
	{
		...
		{
			output_page(\MyBB\View\template('reputation/add_error.twig', [
				'message' => $message,
			]));
			exit;
		}
...
if($mybb->input['action'] == "do_add" && $mybb->request_method == "post")
{
	...
	if(!empty($mybb->input['delete']))
	{
		...
		output_page(\MyBB\View\template('reputation/deleted.twig'));
		exit;
	}
	...
	output_page(\MyBB\View\template('reputation/added.twig'));
	exit;
}
...
if($mybb->input['action'] == "add")
{
	...
	if($mybb->settings['negrep'] || $mybb->settings['neurep'] || $mybb->settings['posrep'])
	{
		...
		output_page(\MyBB\View\template('reputation/add.twig', [
			'user' => $user,
...
if(!$mybb->input['action'])
{
	...
	output_page(\MyBB\View\template('reputation/reputation.twig', [
		'user' => $user,
		'multipage' => $multipage,
