...
if($mybb->user['uid'] != 0)
{
	$logoutlink = eval($templates->render("index_logoutlink"));
}

$statspage = '';
if($mybb->settings['statsenabled'] != 0)
{
	$statspage = eval($templates->render("index_statspage"));
}
...
if($mybb->settings['showwol'] != 0 && $mybb->usergroup['canviewonline'] != 0)
{
		...
		if($user['uid'] > 0)
		{
			...
			if(empty($doneusers[$user['uid']]) || $doneusers[$user['uid']] < $user['time'])
			{
				...
				if($user['invisible'] != 1 || $mybb->usergroup['canviewwolinvis'] == 1 || $user['uid'] == $mybb->user['uid'])
				{
					...
					$onlinemembers .= eval($templates->render("index_whosonline_memberbit", 1, 0));
					$comma = $lang->comma;
				}
	...
	$whosonline = eval($templates->render("index_whosonline"));
}
...
if($mybb->settings['showbirthdays'] != 0)
{
	...
	if(!empty($today_bdays))
	{
		...
		if(!empty($today_bdays))
		{
			foreach($today_bdays as $bdayuser)
			{
				...
				$bdays .= eval($templates->render("index_birthdays_birthday", 1, 0));
				++$bdaycount;
				$comma = $lang->comma;
			}
		}
	...
	{
		$birthdays = eval($templates->render("index_birthdays"));
	}
}
...
if($mybb->settings['showindexstats'] != 0)
{
	...
	$forumstats = eval($templates->render("index_stats"));
}
...
if(($mybb->settings['showwol'] != 0 && $mybb->usergroup['canviewonline'] != 0) || $mybb->settings['showindexstats'] != 0 || ($mybb->settings['showbirthdays'] != 0 && $bdaycount > 0))
{
	...
	$boardstats = eval($templates->render("index_boardstats"));
}
...
$index = eval($templates->render("index"));
output_page($index);
