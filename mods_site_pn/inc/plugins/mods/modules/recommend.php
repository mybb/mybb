...
class Recommend implements Modules
{
	...
	public function run_pre()
	{
		...
		// Collaborators can recommend.
		if ($mybb->user['uid'] == $project['uid'])
		{
			$mods->error($lang->mods_cant_recommend);
		}
		...
		if (!empty($recommend))
		{
			$mods->error($lang->mods_already_recommended);
		}
		
		$db->insert_query('mods_recommended', array('pid' => $pid, 'uid' => (int)$mybb->user['uid']));
