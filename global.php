...
{
	$bbclosedwarning = eval($templates->render("global_boardclosed_warning"));
}
...
if($mybb->user['uid'] != 0)
{
	...
	if($mybb->usergroup['cancp'] == 1 && $mybb->config['hide_admin_links'] != 1)
	{
		...
		$admincplink = eval($templates->render("header_welcomeblock_member_admin"));
	}

	if($mybb->usergroup['canmodcp'] == 1)
	{
		$modcplink = eval($templates->render("header_welcomeblock_member_moderator"));
	}
...
else
{
	...
	$welcomeblock = eval($templates->render("header_welcomeblock_guest"));
}
...
{
	$menu_search = eval($templates->render("header_menu_search"));
	$quicksearch = eval($templates->render("header_quicksearch"));
}

if($mybb->settings['enablememberlist'] == 1 && $mybb->usergroup['canviewmemberlist'] == 1)
{
	$menu_memberlist = eval($templates->render("header_menu_memberlist"));
}

if($mybb->settings['enablecalendar'] == 1 && $mybb->usergroup['canviewcalendar'] == 1)
{
	$menu_calendar = eval($templates->render("header_menu_calendar"));
}

if($mybb->settings['portal'] == 1)
{
	$menu_portal = eval($templates->render("header_menu_portal"));
}
...
if($mybb->user['uid'] != 0 && is_array($groupleaders) && array_key_exists($mybb->user['uid'], $groupleaders))
{
	...
	if($total_joinrequests > 0)
	{
		...
		$pending_joinrequests = eval($templates->render("global_pending_joinrequests"));
	}
}
	...
	if($can_access_moderationqueue || ($mybb->user['ismoderator'] && $mybb->usergroup['canmodcp'] == 1 && $mybb->usergroup['canmanagereportedcontent'] == 1))
	{
		...
		if($reported['unread'] > 0)
		{
			...
			if($unread > 0)
			{
				...
				$unreadreports = eval($templates->render("global_unreadreports"));
			}
		}
	}
}
...
if($mybb->usergroup['isbannedgroup'] == 1)
{
	...
	$bannedwarning = eval($templates->render("global_bannedwarning"));
}
...
if(isset($mybb->user['pmnotice']) && $mybb->user['pmnotice'] == 2 && $mybb->user['pms_unread'] > 0 && $mybb->settings['enablepms'] != 0 && $mybb->usergroup['canusepms'] != 0 && $mybb->usergroup['canview'] != 0 && ($current_page != "private.php" || $mybb->get_input('action') != "read"))
{
	...
	$pm_notice = eval($templates->render("global_pm_alert"));
}
...
if($mybb->usergroup['cancp'] == 1)
{
	...
	if($awaitingusers > 0)
	{
		...
		$awaitingusers = eval($templates->render("global_awaiting_activation"));
	}
	else
	{
		$awaitingusers = '';
...
$gobutton = eval($templates->render("gobutton"));
$htmldoctype = eval($templates->render("htmldoctype", 1, 0));
...
{
	$task_image = eval($templates->render("task_image"));
}
...
if($mybb->settings['showlanguageselect'] != 0)
{
	...
	if(count($languages) > 1)
	{
		foreach($languages as $key => $language)
		{
			...
			$lang_options .= eval($templates->render("footer_languageselect_option"));
		}

		$lang_redirect_url = get_current_location(true, 'language');
		$lang_select = eval($templates->render("footer_languageselect"));
	}
}
...
if($mybb->settings['showthemeselect'] != 0)
{
	...
	if(!empty($theme_options))
	{
		...
		$theme_select = eval($templates->render("footer_themeselect"));
	}
}
...
if(($mybb->settings['contactlink'] == "contact.php" && $mybb->settings['contact'] == 1 && ($mybb->settings['contact_guests'] != 1 && $mybb->user['uid'] == 0 || $mybb->user['uid'] > 0)) || $mybb->settings['contactlink'] != "contact.php")
{
	...
	$contact_us = eval($templates->render("footer_contactus"));
}
...
$footer = eval($templates->render("footer"));
