...
<p><input type=\"text\" name=\"forumperms_per_screen\" value=\"100\" /></p>";
		...
		else
		{
			...
			$query = $this->old_db->simple_select("board_permissions", "*", "ID_GROUP != '-1'", array('limit_start' => $import_session['start_forumperms'], 'limit' => $import_session['forumperms_per_screen']));
...
<p><input type=\"text\" name=\"attachments_per_screen\" value=\"200\" /></p>";
		...
		else
		{
			...
			while($attachment = $this->old_db->fetch_array($query))
			{
				...
				$insert_event['import_aid'] = $event['ID_ATTACH'];
				$insert_event['pid'] = $this->get_import_pid($event['ID_MSG']);
				
				$insert_event['uid'] = $this->get_import_uid($event['ID_MEMBER']);
				...
				$insert_event['filesize'] = $event['size'];
