...
if($mybb->input['action'] == "add")
{
	...
	$form_container->output_row("Time Registered: Type selection", "Select whether the time registered should be counted in hours, days, weeks, months, or years.", $form->generate_select_box("timeregisteredtype", $options, $mybb->input['timeregisteredtype'], array('id' => 'timeregisteredtype')), 'timeregisteredtype');
	...
	$form_container->output_row("Original User Group <em>*</em>", "Select which user group or user groups that the user must be in for the promotion to run. Holding down CTRL selects multiple months. Select 'All User Groups' if you want this promotion to be available for any user group.", $form->generate_select_box('originalusergroup', $options, $mybb->input['originalusergroup'], array('id' => 'originalusergroup', 'multiple' => true)), 'originalusergroup');
...
{	
	...
	while($promotion = $db->fetch_array($query))
	{
		...
		$popup->add_item("Edit Task", "index.php?".SID."&amp;module=users/group_promotions&amp;action=edit&amp;pid={$promotion['pid']}");
		...
		$popup->add_item("Delete Promotion", "index.php?".SID."&amp;module=user/group_promotions&amp;action=delete&amp;pid={$promotion['pid']}");
