...
if($mybb->settings['browsingthisforum'] != "off")
{
	...
	$membercount = 0;
	$inviscount = 0;
	$query = $db->query("SELECT DISTINCT s.ip, s.uid, u.username, s.time, u.invisible, u.usergroup, u.usergroup, u.displaygroup FROM ".TABLE_PREFIX."sessions s LEFT JOIN ".TABLE_PREFIX."users u ON (o.uid=u.uid) WHERE s.time>'$timecut' AND location LIKE '%forumdisplay.php%fid=$fid%' ORDER BY u.username");
