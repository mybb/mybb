...
if($mybb->settings['browsingthisforum'] != "off")
{
	...
	$membercount = 0;
	$inviscount = 0;
	$query = $db->query("SELECT DISTINCT o.ip, o.uid, u.username, o.time, u.invisible, u.usergroup, u.usergroup, u.displaygroup FROM ".TABLE_PREFIX."online o LEFT JOIN ".TABLE_PREFIX."users u ON (o.uid=u.uid) WHERE o.time>'$timecut' AND location LIKE '%forumdisplay.php%fid=$fid%' ORDER BY u.username");
