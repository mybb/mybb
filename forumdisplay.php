...
if ($fpermissions['canviewthreads'] != 0) {
    ...
    if (($datecut > 0 && $datecut != 9999) || isset($fpermissions['canonlyviewownthreads']) && $fpermissions['canonlyviewownthreads'] == 1) {
        ...
        $query = $db->simple_select("forums", "threads, unapprovedthreads, deletedthreads", "fid = '{$fid}'", ['limit' => 1]);
        ...
        if ($threadcount == 0) {
            $query = $db->simple_select("threads", "COUNT(tid) AS threads", "fid = '$fid' $useronly $visibleonly", ['limit' => 1]);
            $threadcount = $db->fetch_field($query, "threads");
        }
    }
}
