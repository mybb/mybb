...
if($mybb->input['action'] == "thread")
{
	...
	if($mybb->settings['showsimilarthreads'] != "no")
	{
		$query = $db->query("
			...
			WHERE t.fid='{$thread['fid']}' AND t.tid!='{$thread['tid']}' AND t.visible='1' AND MATCH (t.subject) AGAINST ('".$db->escape_string($thread['subject'])."') >= '{$mybb->settings['similarityrating']}'
