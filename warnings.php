...
if($mybb->input['action'] == "warn")
{
	...
	if($mybb->get_input('pid', 1))
	{
		...
		$post = eval($templates->render("warnings_warn_post"));
		...
		while($warning = $db->fetch_array($query))
		{
			if($warning['expired'] != $last_expired || $first)
			{
				...
				{
					$warnings .= eval($templates->render("warnings_active_header"));
				}
				else
				{
					$warnings .= eval($templates->render("warnings_expired_header"));
				}
			}
			...
			$warnings .= eval($templates->render("warnings_warning"));
		}
		if($warnings)
		{
			$existing_warnings = eval($templates->render("warnings_warn_existing"));
		}
	}
	...
	while($type = $db->fetch_array($query))
	{
		...
		{
			$result = eval($templates->render("warnings_warn_type_result"));
		}
		$types .= eval($templates->render("warnings_warn_type"));
		unset($new_level);
		unset($result);
	}
	...
	if($mybb->settings['allowcustomwarnings'] != 0)
	{
		...
		$custom_warning = eval($templates->render("warnings_warn_custom"));
	}
	...
	if($group_permissions['canusepms']  != 0 && $mybb->user['receivepms'] != 0 && $mybb->settings['enablepms'] != 0)
	{
		...
		if($mybb->settings['allowanonwarningpms'] == 1)
		{
			...
			$anonymous_pm = eval($templates->render("warnings_warn_pm_anonymous"));
		}

		$pm_notify = eval($templates->render("warnings_warn_pm"));
	}

	$plugins->run_hooks("warnings_warn_end");

	$warn = eval($templates->render("warnings_warn"));
	output_page($warn);
	exit;
}
...
if($mybb->input['action'] == "view")
{
	...
	if($warning['post_subject'])
	{
		...
		$warning_info = eval($templates->render("warnings_view_post"));
	}
	else
	{
		$warning_info = eval($templates->render("warnings_view_user"));
	}
	...
	if(!$warning['daterevoked'])
	{
		...
		$revoke = eval($templates->render("warnings_view_revoke"));
	}
	else
	{
		...
		$revoke = eval($templates->render("warnings_view_revoked"));
	}

	$plugins->run_hooks("warnings_view_end");

	$warning = eval($templates->render("warnings_view"));
	output_page($warning);
}
...
if(!$mybb->input['action'])
{
	...
	if(!$warnings)
	{
		$warnings = eval($templates->render("warnings_no_warnings"));
	}

	$plugins->run_hooks("warnings_end");

	$warnings = eval($templates->render("warnings"));
