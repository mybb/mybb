...
elseif($mybb->input['action'] == "joinrequests")
{
	...
	while($user = $db->fetch_array($query))
	{
		...
		eval($templates->render("managegroup_joinrequests_request", "users", true));
	}
	if(!$users)
	{
	...
	eval($templates->render("managegroup_joinrequests", "joinrequests"));
...
else
{
	...
	elseif($usergroup['type'] == 4)
	{
		...
		if($numrequests['req'])
		{
			...
			eval($templates->render("managegroup_requestnote", "joinrequests"));
		}
	...
	if($db->num_rows($query))
	{
		...
		while($leader = $db->fetch_array($query))
		{
			...
			++$loop;
			eval($templates->render("managegroup_leaders_bit", "leaders", true));
		}

		eval($templates->render("managegroup_leaders", "group_leaders"));
	}
	...
	while($user = $db->fetch_array($query))
	{
		...
		{
			eval($templates->render("postbit_pm", "sendpm"));
		}

		if($user['hideemail'] != 1)
		{
			eval($templates->render("postbit_email", "email"));
		}
		else
		{
			$email = '';
		...
		{
			eval($templates->render("managegroup_user_checkbox", "checkbox"));
		}

		eval($templates->render("managegroup_user", "users", true));
	}

	if(!$users)
	{
		eval($templates->render("managegroup_no_users", "users"));
	}
	...
	if($groupleader['canmanagemembers'] == 1)
	{
		eval($templates->render("managegroup_adduser", "add_user"));
		eval($templates->render("managegroup_removeusers", "remove_users"));
	}

	if($usergroup['type'] == 5 && $groupleader['caninvitemembers'] == 1)
	{
		eval($templates->render("managegroup_inviteuser", "invite_user"));
	}

	$plugins->run_hooks("managegroup_end");

	eval($templates->render("managegroup", "manageusers"));
