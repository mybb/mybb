...
function build_postbit($post, $post_type=0)
{
	...
	if($post['userusername'])
	{
		...
		$post['button_rep'] = '';
		if($post_type != 3 && $mybb->settings['enablereputation'] == 1 && $mybb->settings['postrep'] == 1 && $mybb->usergroup['cangivereputations'] == 1 && $usergroup['usereputationsystem'] == 1 && ($mybb->settings['posrep'] || $mybb->settings['neurep'] || $mybb->settings['negrep']) && $post['uid'] != $mybb->user['uid'] && $post['visible'] == 1 && $thread['visible'] == 1)
		{
			if(!$post['pid'])
			{
				$post['pid'] = 0;
