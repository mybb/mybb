...
function build_postbit($post, $post_type=0)
{
	...
	if(!$post_type)
	{
		...
		if($post['edituid'] != 0 && $post['edittime'] != 0 && $post['editusername'] != "" && (($mybb->settings['showeditedby'] != 0 && $usergroup['cancp'] == 0) || ($mybb->settings['showeditedbyadmin'] != 0 && $usergroup['cancp'] == || is_moderator($post['fid'], "", $post['uid'])))))
		{
			$post['editedmsg'] = true;
