...
function build_postbit($post, $post_type=0)
{
	...
	if(!empty($usergroup['image']))
	{
		...
		eval($templates->render("postbit_groupimage", "post['groupimage']"));
	...
	if($post['userusername'])
	{
		...
		if($post['starimage'] && $post['stars'])
		{
			...
			{
				eval($templates->render("postbit_userstar", "post['userstars']", true, 1, 0));
			}
		...
		{
			eval($templates->render("postbit_online", "post['onlinestatus']"));
		}
		else
		{
			if($post['away'] == 1 && $mybb->settings['allowaway'] != 0)
			{
				eval($templates->render("postbit_away", "post['onlinestatus']"));
			}
			else
			{
				eval($templates->render("postbit_offline", "post['onlinestatus']"));
			}
		}
		...
		if(isset($mybb->user['showavatars']) && $mybb->user['showavatars'] != 0 || $mybb->user['uid'] == 0)
		{
			...
			eval($templates->render("postbit_avatar", "post['useravatar']"));
		}
		else
		{
			$post['useravatar'] = '';
		...
		eval($templates->render("postbit_find", "post['button_find']"));

		if($mybb->settings['enablepms'] == 1 && $post['receivepms'] != 0 && $mybb->usergroup['cansendpms'] == 1 && my_strpos(",".$post['ignorelist'].",", ",".$mybb->user['uid'].",") === false)
		{
			eval($templates->render("postbit_pm", "post['button_pm']"));
		}
		...
		if($post_type != 3 && $mybb->settings['enablereputation'] == 1 && $mybb->settings['postrep'] == 1 && $mybb->usergroup['cangivereputations'] == 1 && $usergroup['usereputationsystem'] == 1 && ($mybb->settings['posrep'] || $mybb->settings['neurep'] || $mybb->settings['negrep']) && $post['uid'] != $mybb->user['uid'])
		{
			...
			eval($templates->render("postbit_rep_button", "post['button_rep']"));
		}
		...
		if($post['website'] != "" && $mybb->settings['hidewebsite'] != -1 && !is_member($mybb->settings['hidewebsite']) && $usergroup['canchangewebsite'] == 1)
		{
			...
			eval($templates->render("postbit_www", "post['button_www']"));
		}
		else
		{
			$post['button_www'] = "";
		...
		{
			eval($templates->render("postbit_email", "post['button_email']"));
		}
		else
		{
			$post['button_email'] = "";
		...
		if($post_type != 3 && $usergroup['usereputationsystem'] != 0 && $mybb->settings['enablereputation'] == 1)
		{
			...
			eval($templates->render("postbit_reputation", "post['replink']"));
		}
		...
		if($post_type != 3 && $mybb->settings['enablewarningsystem'] != 0 && $usergroup['canreceivewarnings'] != 0 && ($mybb->usergroup['canwarnusers'] != 0 || ($mybb->user['uid'] == $post['uid'] && $mybb->settings['canviewownwarning'] != 0)))
		{
			...
			{
				eval($templates->render("postbit_warn", "post['button_warn']"));
			...
			eval($templates->render("postbit_warninglevel", "post['warninglevel']"));
		}

		if($post_type != 3 && $post_type != 1 && purgespammer_show($post['postnum'], $post['usergroup'], $post['uid']))
		{
			eval($templates->render('postbit_purgespammer', "post['button_purgespammer']"));
		}
				...
				if(!empty($post[$fieldfid]))
				{
					...
					if(is_array($useropts) && ($type == "multiselect" || $type == "checkbox"))
					{
						foreach($useropts as $val)
						{
							if($val != '')
							{
								eval($templates->render("postbit_profilefield_multiselect_value", "post['fieldvalue_option']", true));
							}
						}
						if($post['fieldvalue_option'] != '')
						{
							eval($templates->render("postbit_profilefield_multiselect", "post['fieldvalue']", true));
						}
					}
					else
					{
					...
					eval($templates->render("postbit_profilefield", "post['profilefield']", true));
				}
			}
		}

		eval($templates->render("postbit_author_user", "post['user_details']"));
	}
	else
	{ // Message was posted by a guest or an unknown user
		...
		$post['onlinestatus'] = '';
		$post['replink'] = '';
		eval($templates->render("postbit_author_guest", "post['user_details']"));
	}
	...
	if($post_type == 2 && $post['pmid'])
	{
		...
		eval($templates->render("postbit_reply_pm", "post['button_reply_pm']"));
		eval($templates->render("postbit_forward_pm", "post['button_forward_pm']"));
		eval($templates->render("postbit_delete_pm", "post['button_delete_pm']"));

		if($replyall == true)
		{
			eval($templates->render("postbit_replyall_pm", "post['button_replyall_pm']"));
		}
	}
	...
	if(!$post_type)
	{
		...
		if($post['edituid'] != 0 && $post['edittime'] != 0 && $post['editusername'] != "" && (($mybb->settings['showeditedby'] != 0 && $usergroup['cancp'] == 0) || ($mybb->settings['showeditedbyadmin'] != 0 && $usergroup['cancp'] == 1)))
		{
			...
			eval($templates->render("postbit_editedby", "post['editedmsg']"));
		}

		if((is_moderator($fid, "caneditposts") || ($forumpermissions['caneditposts'] == 1 && $mybb->user['uid'] == $post['uid'])) && $mybb->user['uid'] != 0)
		{
			eval($templates->render("postbit_edit", "post['button_edit']"));
		}
		...
		if($mybb->user['uid'] != 0)
		{
			if((is_moderator($fid, "candeleteposts") || $can_delete_post == 1) && $postcounter != 1)
			{
				...
				eval($templates->render("postbit_quickdelete", "post['button_quickdelete']"));
				...
				if(is_moderator($fid, "canrestoreposts"))
				{
					...
					eval($templates->render("postbit_quickrestore", "post['button_quickrestore']"));
				}
			}
		...
		if($ismod)
		{
			...
			eval($templates->render("postbit_inlinecheck", "post['inlinecheck']"));
		...
		eval($templates->render("postbit_posturl", "post['posturl']"));
		global $forum, $thread;
		...
		{
			eval($templates->render("postbit_quote", "post['button_quote']"));
		}
		...
		{
			eval($templates->render("postbit_multiquote", "post['button_multiquote']"));
		}

		if($mybb->user['uid'] != "0")
		{
			eval($templates->render("postbit_report", "post['button_report']"));
		}
	}
	elseif($post_type == 3) // announcement
	{
		...
		{
			eval($templates->render("announcement_edit", "post['button_edit']"));
			eval($templates->render("announcement_quickdelete", "post['button_quickdelete']"));
		}
	}
	...
	if(!$post_type || $post_type == 2)
	{
		if($show_ips != "no" && !empty($post['ipaddress']))
		{
			if($show_ips == "show")
			{
				eval($templates->render("postbit_iplogged_show", "post['iplogged']"));
			}
			else if($show_ips == "hide" && (is_moderator($fid, "canviewips") || $mybb->usergroup['issupermod']))
			{
				...
				eval($templates->render("postbit_iplogged_hiden", "post['iplogged']"));
			}
		}
	}
	...
	if(isset($post['includesig']) && $post['includesig'] != 0 && $post['username'] && $post['signature'] != "" && ($mybb->user['uid'] == 0 || $mybb->user['showsigs'] != 0) && ($post['suspendsignature'] == 0 || $post['suspendsignature'] == 1 && $post['suspendsigtime'] != 0 && $post['suspendsigtime'] < TIME_NOW) && $usergroup['canusesig'] == 1 && ($usergroup['canusesigxposts'] == 0 || $usergroup['canusesigxposts'] > 0 && $postnum > $usergroup['canusesigxposts']) && $mybb->settings['hidesignatures'] != -1 && !is_member($mybb->settings['hidesignatures']))
	{
		...
		eval($templates->render("postbit_signature", "post['signature']"));
	}
	else
	{
		$post['signature'] = "";
	...
	if(isset($post['icon']) && $post['icon'] > 0 && $icon_cache[$post['icon']])
	{
		...
		eval($templates->render("postbit_icon", "post['icon']"));
	}
	else
	{
		$post['icon'] = "";
	...
	switch($post_type)
	{
		...
		default: // Regular post
			...
			if(is_array($ignored_users) && $post['uid'] != 0 && isset($ignored_users[$post['uid']]) && $ignored_users[$post['uid']] == 1)
			{
				...
				eval($templates->render("postbit_ignored", "ignore_bit"));
	...
	if($mybb->settings['postlayout'] == "classic")
	{
		eval($templates->render("postbit_classic", "postbit"));
	}
	else
	{
		eval($templates->render("postbit", "postbit"));
	}
	$GLOBALS['post'] = "";
...
function get_post_attachments($id, &$post)
{
	...
	{ // This post has 1 or more attachments
		foreach($attachcache[$id] as $aid => $attachment)
		{
			...
			{ // There is an attachment thats visible!
				...
				if(stripos($post['message'], "[attachment=".$attachment['aid']."]") !== false)
				{
					...
					{
						eval($templates->render("postbit_attachments_thumbnails_thumbnail", "attbit"));
					}
					elseif((($attachment['thumbnail'] == "SMALL" && $forumpermissions['candlattachments'] == 1) || $mybb->settings['attachthumbnails'] == "no") && $isimage)
					{
						eval($templates->render("postbit_attachments_images_image", "attbit"));
					}
					else
					{
						eval($templates->render("postbit_attachments_attachment", "attbit"));
					}
				...
				else
				{
					...
					if($attachment['thumbnail'] != "SMALL" && $attachment['thumbnail'] != "" && $mybb->settings['attachthumbnails'] == "yes")
					{
						eval($templates->render("postbit_attachments_thumbnails_thumbnail", "post['thumblist']", true));
						if($tcount == 5)
						{
							$thumblist .= "<br />";
							$tcount = 0;
					...
					elseif((($attachment['thumbnail'] == "SMALL" && $forumpermissions['candlattachments'] == 1) || $mybb->settings['attachthumbnails'] == "no") && $isimage)
					{
						eval($templates->render("postbit_attachments_images_image", "post['imagelist']", true));
					}
					else
					{
						eval($templates->render("postbit_attachments_attachment", "post['attachmentlist']", true));
					}
				}
			}
			else
		...
		if($validationcount > 0 && is_moderator($post['fid'], "canviewunapprove"))
		{
			...
			eval($templates->render("postbit_attachments_attachment_unapproved", "post['attachmentlist']", true));
		}
		if($post['thumblist'])
		{
			eval($templates->render("postbit_attachments_thumbnails", "post['attachedthumbs']"));
		}
		else
		{
			$post['attachedthumbs'] = '';
		...
		if($post['imagelist'])
		{
			eval($templates->render("postbit_attachments_images", "post['attachedimages']"));
		}
		else
		{
			$post['attachedimages'] = '';
		...
		{
			eval($templates->render("postbit_attachments", "post['attachments']"));
		}
	}
}
