...
class postParser
{
	...
	function cache_smilies()
	{
		...
		if(is_array($smilies))
		{
			...
			foreach($smilies as $sid => $smilie)
			{
				...
				{
					$s = $this->parse_html($s);
					$smilie_template = eval($templates->render("smilie", 1, 0));
	...
	*/
	function mycode_parse_post_quotes($message, $username, $text_only=false)
	{
		...
		if((int)$match[1])
		{
			...
			{
				$linkback = " " . eval($templates->render("postbit_gotopost", 1, 0));
			}
	...
	*/
	function mycode_parse_video($video, $url)
	{
		...
		$video_code = eval($templates->render("video_{$video}_embed"));
