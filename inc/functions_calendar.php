...
function build_mini_calendar($calendar, $month, $year, &$events_cache)
{
	...
	foreach($weekdays as $weekday)
	{
		...
		eval($templates->render("calendar_mini_weekdayheader", "weekday_headers", true));
	}
	...
	for($row = 0; $row < 6; ++$row) // Iterate weeks (each week gets a row)
	{
		foreach($weekdays as $weekday_id => $weekday)
		{
			...
			eval($templates->render("calendar_mini_weekrow_day", "day_bits", true));
			++$day;
		}
		if($day_bits)
		{
			eval($templates->render("calendar_mini_weekrow", "calendar_rows", true));
		}
		$day_bits = "";
	}
	eval($templates->render("calendar_mini", "mini_calendar"));
...
function build_calendar_jump($selected=0)
{
	...
	foreach($calendars as $calendar)
	{
		...
		eval($templates->render("calendar_jump_option", "jump_options", true));
	}

	eval($templates->render("calendar_jump", "calendar_jump"));
