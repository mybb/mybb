...
function usercp_menu()
{
	...
	eval($templates->render("usercp_nav", "usercpnav"));
...
function usercp_menu_messenger()
{
	...
	{
		eval($templates->render("usercp_nav_messenger_compose", "ucp_nav_compose"));
	}
	...
	foreach($foldersexploded as $key => $folders)
	{
		...
		eval($templates->render("usercp_nav_messenger_folder", "folderlinks", true));
	}
...
function usercp_menu_profile()
{
	...
	{
		eval($templates->render("usercp_nav_changename", "changenameop"));
	}
	...
	if($mybb->usergroup['canusesig'] == 1 && ($mybb->usergroup['canusesigxposts'] == 0 || $mybb->usergroup['canusesigxposts'] > 0 && $mybb->user['postnum'] > $mybb->usergroup['canusesigxposts']))
	{
		...
		{
			eval($templates->render("usercp_nav_editsignature", "changesigop"));
		}
	}
	...
	eval($templates->render("usercp_nav_profile", "usercpmenu", true));
}
...
function usercp_menu_misc()
{
	...
	{
		eval($templates->render("usercp_nav_attachments", "attachmentop"));
	}
	...
	eval($templates->render("usercp_nav_misc", "usercpmenu", true));
}
