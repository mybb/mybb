...
function output_page($contents)
{
	...
	if($mybb->usergroup['cancp'] == 1 || $mybb->dev_mode == 1)
	{
		if($mybb->settings['extraadmininfo'] != 0)
		{
			...
			$debugstuff = eval($templates->render("debug_summary"));
...
function error($error="", $title="")
{
	...
	$errorpage = eval($templates->render("error"));
...
function error_no_permission()
{
	...
	if($mybb->user['uid'])
	{
		...
		$errorpage = eval($templates->render("error_nopermission_loggedin"));
	}
	else
	{
		...
		$errorpage = eval($templates->render("error_nopermission"));
	}
...
function redirect($url, $message="", $title="", $force_redirect=false)
{
	...
	if($force_redirect == true || ($mybb->settings['redirects'] == 1 && ($mybb->user['showredirect'] == 1 || !$mybb->user['uid'])))
	{
		...
		$redirectpage = eval($templates->render("redirect"));
...
function multipage($count, $perpage, $page, $url, $breadcrumb=false)
{
	...
	if($page > 1)
	{
		...
		$prevpage = eval($templates->render("multipage_prevpage"));
	}
	...
	if($from > 1)
	{
		...
		$start = eval($templates->render("multipage_start"));
	}
	...
	for($i = $from; $i <= $to; ++$i)
	{
		...
		if($page == $i)
		{
			if($breadcrumb == true)
			{
				$mppage .= eval($templates->render("multipage_page_link_current"));
			}
			else
			{
				$mppage .= eval($templates->render("multipage_page_current"));
			}
		}
		else
		{
			$mppage .= eval($templates->render("multipage_page"));
		}
	}
	...
	if($to < $pages)
	{
		...
		$end = eval($templates->render("multipage_end"));
	}
	...
	if($page < $pages)
	{
		...
		$nextpage = eval($templates->render("multipage_nextpage"));
	}
	...
	if($pages > ($mybb->settings['maxmultipagelinks']+1) && $mybb->settings['jumptopagemultipage'] == 1)
	{
		...
		$jumptopage = eval($templates->render("multipage_jump_page"));
	}
	...
	if($breadcrumb == true)
	{
		$multipage = eval($templates->render("multipage_breadcrumb"));
	}
	else
	{
		$multipage = eval($templates->render("multipage"));
	}
...
function check_forum_password($fid, $pid=0)
{
	...
	if(!empty($forum_cache[$fid]['password']))
	{
		...
		if(isset($mybb->input['pwverify']) && $pid == 0)
		{
			...
			{
				$pwnote = eval($templates->render("forumdisplay_password_wrongpass"));
				$showform = true;
			}
		}
		else
	...
	if($showform)
	{
		...
		else
		{
			...
			$pwform = eval($templates->render("forumdisplay_password"));
			output_page($pwform);
		}
		exit;
	}
...
function get_post_icons()
{
	...
	foreach($posticons as $dbicon)
	{
		...
		$iconlist .= eval($templates->render("posticons_icon"));
	}

	$posticons = eval($templates->render("posticons"));
...
function build_forum_jump($pid="0", $selitem="", $addselect="1", $depth="", $showextras="1", $showall=false, $permissions="", $name="fid")
{
		...
		foreach($jumpfcache[$pid] as $main)
		{
			foreach($main as $forum)
			{
				...
				if($forum['fid'] != "0" && ($perms['canview'] != 0 || $mybb->settings['hideprivateforums'] == 0) && $forum['linkto'] == '' && ($forum['showinjump'] != 0 || $showall == true))
				{
					...
					$forumjumpbits .= eval($templates->render("forumjump_bit"));
	...
	if($addselect)
	{
		...
		$forumjump = eval($templates->render("forumjump_".$template));
	}
...
function build_mycode_inserter($bind="message", $smilies = true)
{
	...
	if($mybb->settings['bbcodeinserter'] != 0)
	{
		...
		else
		{
			...
			$codeinsert = eval($templates->render("codebuttons"));
		}
	}
	...
	if($mybb->settings['smilieinserter'] != 0 && $mybb->settings['smilieinsertercols'] && $mybb->settings['smilieinsertertot'])
	{
		...
		if(is_array($smiliecache))
		{
			...
			else if($mybb->settings['smilieinsertertot'] < $smiliecount)
			{
				...
				$getmore = eval($templates->render("smilieinsert_getmore"));
			}
			...
			foreach($smiliecache as $smilie)
			{
				if($i < $mybb->settings['smilieinsertertot'])
				{
					...
					$smilie = eval($templates->render("smilie", 1, 0));
					$smilies .= eval($templates->render("smilieinsert_smilie"));
					++$i;
					++$counter;
			...
			$clickablesmilies = eval($templates->render("smilieinsert"));
		}
		else
		{
			$clickablesmilies = "";
...
function build_prefix_select($fid, $selected_pid=0, $multiple=0)
{
	...
	foreach($prefixes as $prefix)
	{
		...
		$prefixselect_prefix .= eval($templates->render("post_prefixselect_prefix"));
	}

	if($multiple != 0)
	{
		$prefixselect = eval($templates->render("post_prefixselect_multiple"));
	}
	else
	{
		$prefixselect = eval($templates->render("post_prefixselect_single"));
	}
...
function build_forum_prefix_select($fid, $selected_pid=0)
{
	...
	foreach($prefixes as $prefix)
	{
		...
		$prefix['prefix'] = htmlspecialchars_uni($prefix['prefix']);
		$prefixselect_prefix .= eval($templates->render("forumdisplay_threadlist_prefixes_prefix"));
	}

	$prefixselect = eval($templates->render("forumdisplay_threadlist_prefixes"));
	return $prefixselect;
}
...
function get_reputation($reputation, $uid=0)
{
	...
	if($uid != 0)
	{
		$display_reputation = eval($templates->render("postbit_reputation_formatted_link"));
	}
	else
	{
		$display_reputation = eval($templates->render("postbit_reputation_formatted"));
	}
...
function get_colored_warning_level($level)
{
	...
	$level = eval($templates->render("postbit_warninglevel_formatted"));
	return $level;
}
...
function get_attachment_icon($ext)
{
	...
	$attachment_icon = eval($templates->render("attachment_icon"));
...
function build_breadcrumb()
{
	...
	$navsep = eval($templates->render("nav_sep"));
		...
		foreach($navbits as $key => $navbit)
		{
			if(isset($navbits[$key+1]))
			{
				...
				if(!empty($navbit['multipage']))
				{
					...
					{
						++$i;
						$multipage_dropdown = eval($templates->render("nav_dropdown"));
				...
				$nav .= eval($templates->render("nav_bit"));
			}
		}
	}
	...
	if($nav)
	{
		$activesep = eval($templates->render("nav_sep_active"));
	}

	$activebit = eval($templates->render("nav_bit_active"));
	$donenav = eval($templates->render("nav"));
...
function build_theme_select($name, $selected="", $tid=0, $depth="", $usergroup_override=false, $footer=false, $count_override=false)
{
		...
		foreach($tcache[$tid] as $theme)
		{
			...
			if($is_allowed || $theme['allowedgroups'] == "all" || $usergroup_override == true)
			{
				...
				if($theme['pid'] != 0)
				{
					...
					$themeselect_option .= eval($templates->render("usercp_themeselector_option"));
					++$num_themes;
					$depthit = $depth."--";
				}
	...
	if($tid == 1 && ($num_themes > 1 || $count_override == true))
	{
		if($footer == true)
		{
			$themeselect = eval($templates->render("footer_themeselector"));
		}
		else
		{
			$themeselect = eval($templates->render("usercp_themeselector"));
		}
...
function build_timezone_select($name, $selected=0, $short=false)
{
	...
	foreach($timezones as $timezone => $label)
	{
		...
		$timezone_option .= eval($templates->render("usercp_options_timezone_option"));
	}

	$select = eval($templates->render("usercp_options_timezone"));
	return $select;
}
