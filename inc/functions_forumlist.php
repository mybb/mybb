...
function build_forumbits($pid=0, $depth=1)
{
	...
	foreach($fcache[$pid] as $parent)
	{
		foreach($parent as $forum)
		{
			...
			{
				eval($templates->render("forumbit_subforums", "subforums"));
			}
			...
			else if($depth == 3)
			{
				if($donecount < $mybb->settings['subforumsindex'])
				{
					...
					if($mybb->settings['subforumsstatusicons'] == 1)
					{
						...
						eval($templates->render("forumbit_depth3_statusicon", "statusicon", false, 1, 0));
					}

					// Fetch the template and append it to the list
					eval($templates->render("forumbit_depth3", "forum_list", true, 1, 0));
					$comma = $lang->comma;
				}
			...
			if($forum['linkto'] == '')
			{
				...
				elseif($hideinfo != true)
				{
					...
					if($depth != 1)
					{
						eval($templates->render("forumbit_depth{$depth}_forum_lastpost", "lastpost"));
					}
				}
				...
				if($mybb->settings['showforumviewing'] != 0 && $forum['viewers'] > 0)
				{
					...
					eval($templates->render("forumbit_depth2_forum_viewers", "forum_viewers_text"));
				}
			}
			...
			if($mybb->settings['modlist'] != 0)
			{
				...
				if($moderators)
				{
					eval($templates->render("forumbit_moderators", "modlist"));
				}
				else
				{
					$modlist = '';
			...
			eval($templates->render("forumbit_depth$depth$forumcat", "forum_list", true));
		}
	}
...
function get_forum_unapproved($forum)
{
	...
	if(is_moderator($forum['fid'], "canviewunapprove"))
	{
		...
		if($forum['unapprovedposts'])
		{
			...
			eval($templates->render("forumbit_depth2_forum_unapproved_posts", "unapproved_posts"));
		}
		...
		if($forum['unapprovedthreads'])
		{
			...
			eval($templates->render("forumbit_depth2_forum_unapproved_threads", "unapproved_threads"));
		}
	}
	return array(
