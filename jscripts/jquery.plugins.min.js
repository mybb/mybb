...
return this;};$(document).on('click.modal','a[rel="modal:close"]',$.modal.close);$(document).on('click.modal','a[rel="modal:open"]',function(event){event.preventDefault();$(this).modal();});}));(function($){var current_popup='';var PopupMenu=function(el,close_in_popupmenu)
{var el=$(el);var popup=this;var popup_menu=$("#"+el.attr('id')+"_popup");if(typeof close_in_popupmenu=='undefined')
{var close_in_popupmenu=true;}
this.open=function(e)
{e.preventDefault();if(popup_menu.is(':visible'))
{popup.close();return;}
var offset=el.offset();offset.top+=el.outerHeight();if((el.offset().left+popup_menu.outerWidth())>$(window).width())
var adjust=popup_menu.outerWidth()-el.outerWidth();else
var adjust=0;popup_menu.css({position:'absolute',top:offset.top,left:offset.left-adjust});popup_menu.show();$('body, .popup_item').bind('click.close_popup',function(e){if(close_in_popupmenu)
{if($(e.target).closest("#"+el.attr('id')).length==0){popup.close();}}
else
{if($(e.target).closest("#"+el.attr('id')).length==0&&$(e.target).closest("#"+el.attr('id')+'_popup').length==0){popup.close();}}});}
this.close=function(e)
{popup_menu.hide();}}
$.fn.popupMenu=function(close_in_popupmenu)
{return this.each(function()
{var popup=new PopupMenu(this,close_in_popupmenu);$(this).click(popup.open);});}})(jQuery);/*! jQuery-Impromptu - v6.2.3 - 2016-04-23
* http://trentrichardson.com/Impromptu
* Copyright (c) 2016 Trent Richardson; Licensed MIT */(function(root,factory){if(typeof define==='function'&&define.amd){define(['jquery'],factory);}else{factory(root.jQuery);}}(this,function($){'use strict';$.prompt=function(message,options){var html=`<div class="modal">
	<div style="overflow-y: auto; max-height: 400px;" class="modal__class">
		<table border="0" cellspacing="0" cellpadding="5" class="tborder">
			<tr>
				<td class="thead" colspan="2"><strong>modal__text</strong></td>
			</tr>
			<tr>
				<td colspan="2" class="trow1">
				<div style="text-align: center" class="modal_buttons">modal__buttons</div></td>
			</tr>
		</table>
	</div>
</div>`;var defaults={fadeDuration:250,zIndex:(typeof modal_zindex!=='undefined'?modal_zindex:9999)};var buttonsText='';for(var i in options.buttons){buttonsText+='<button class="button">'+options.buttons[i].title+'</button>';}
var modal=$(html.replace('modal__text',message).replace('modal__buttons',buttonsText));modal.modal($.extend(defaults,options));var buttons=modal.find('.modal_buttons > button');buttons.click(function(e){var index=$(this).index();if(options.submit(e,options.buttons[index].value)==false)
return;$.modal.close();});return modal;};$.fn.prompt=function(options){if(options===undefined){options={};}
