...
else if($mybb->input['action'] == "edit_post")
{
	...
	if($mybb->get_input('do') == "get_post")
	{
		...
		/*$inline_editor = eval($templates->render("xmlhttp_inline_post_editor"));
	...
	else if($mybb->get_input('do') == "update_post")
	{
		...
		if($mybb->settings['showeditedby'] != 0 && $mybb->settings['showeditedbyadmin'] != 0)
		{
			...
			$editedmsg = eval($templates->render("postbit_editedby"));
		}
...
else if($mybb->input['action'] == "get_buddyselect")
{
	...
	if($mybb->user['buddylist'] != "")
	{
		...
		while($buddy = $db->fetch_array($query))
		{
			...
			{
				$online[] = eval($templates->render("xmlhttp_buddyselect_online"));
			}
			else
			{
				$offline[] = eval($templates->render("xmlhttp_buddyselect_offline"));
			}
		}
		...
		$buddy_select = eval($templates->render("xmlhttp_buddyselect"));
		echo $buddy_select;
	}
	else
	{
