...
while($postrow = $db->fetch_array($query))
{
	...
	$postrows .= eval($templates->render("printthread_post"));
}

$plugins->run_hooks("printthread_end");

$printable = eval($templates->render("printthread"));
...
function printthread_multipage($count, $perpage, $current_page, $url)
{
	...
	if($count > $perpage)
	{
		...
		for($page = 1; $page <= $pages; ++$page)
		{
			...
			{
				$mppage .= eval($templates->render("printthread_multipage_page_current"));
			}
			else
			{
				$mppage .= eval($templates->render("printthread_multipage_page"));
			}
		}

		$multipage = eval($templates->render("printthread_multipage"));
	}
	return $multipage;
}
