		...
		foreach($forumcache[$pid] as $key => $main)
		{
			foreach($main as $key => $forum)
			{
				...
				if($perms['canview'] == 1 || $mybb->settings['hideprivateforums'] == 0)
				{
					...
					if($forum['password'] == '' && !in_array($forum['fid'], $unexp) || $forum['password'] && isset($mybb->cookies['forumpass'][$forum['fid']]) && hash_equals($mybb->cookies['forumpass'][$forum['fid']], md5($mybb->user['uid'].$forum['password'])))
					{
						$forum['depth'] = $depth;
						$forumlist[] = $forum;
					}
