...
elseif($mybb->input['action'] == "rules")
{
	if(isset($mybb->input['fid']))
	{
		...
		$rules = eval($templates->render("misc_rules_forum"));
		output_page($rules);
	}
...
elseif($mybb->input['action'] == "helpresults")
{
	...
	while($helpdoc = $db->fetch_array($query))
	{
		...
		$helpdoclist .= eval($templates->render("misc_helpresults_bit"));
	}

	if($db->num_rows($query) == 0)
	{
		$helpdoclist = eval($templates->render("misc_helpresults_noresults"));
	}

	$plugins->run_hooks("misc_helpresults_end");

	$helpresults = eval($templates->render("misc_helpresults"));
...
elseif($mybb->input['action'] == "help")
{
	...
	if($hid)
	{
		...
		if($helpdoc['section'] != 0 && $helpdoc['enabled'] != 0)
		{
			...
			$helppage = eval($templates->render("misc_help_helpdoc"));
	...
	else
	{
		...
		while($section = $db->fetch_array($query))
		{
			...
			if(is_array($helpdocs[$section['sid']]))
			{
				...
				foreach($helpdocs[$section['sid']] as $key => $bit)
				{
					foreach($bit as $key => $helpdoc)
					{
						if($helpdoc['enabled'] != 0)
						{
							...
							$altbg = alt_trow();
							$helpbits .= eval($templates->render("misc_help_section_bit"));
						}
					}
					$expdisplay = '';
				...
				$sections .= eval($templates->render("misc_help_section"));
			}
		}

		if($mybb->settings['helpsearch'] == 1)
		{
			$search = eval($templates->render("misc_help_search"));
		}

		$plugins->run_hooks("misc_help_section_end");

		$help = eval($templates->render("misc_help"));
		output_page($help);
	}
}
elseif($mybb->input['action'] == "buddypopup")
{
	...
	if($mybb->user['buddylist'] != "")
	{
		...
		while($buddy = $db->fetch_array($query))
		{
			...
			{
				$send_pm = eval($templates->render("misc_buddypopup_user_sendpm"));
			}
			...
			if($buddy['lastactive'] > $timecut && ($buddy['invisible'] == 0 || $mybb->user['usergroup'] == 4) && $buddy['lastvisit'] != $buddy['lastactive'])
			{
				...
				$buddys['online'] .= eval($templates->render("misc_buddypopup_user_online"));
			}
			else
			{
				$boffline_alt = alt_trow();
				$buddys['offline'] .= eval($templates->render("misc_buddypopup_user_offline"));
			}
		}
		...
		if(empty($buddys['online']))
		{
			...
			$buddys['online'] = eval($templates->render("misc_buddypopup_user_none"));
		}

		if(empty($buddys['offline']))
		{
			$error = $lang->offline_none;
			$buddys['offline'] = eval($templates->render("misc_buddypopup_user_none"));
		}

		$buddies = eval($templates->render("misc_buddypopup_user"));
	}
	else
	{
		// No buddies? :(
		...
		$buddies = eval($templates->render("misc_buddypopup_user_none"));
	}

	$plugins->run_hooks("misc_buddypopup_end");

	$buddylist = eval($templates->render("misc_buddypopup", 1, 0));
	echo $buddylist;
	exit;
}
elseif($mybb->input['action'] == "whoposted")
{
	...
	while($poster = $db->fetch_array($query))
	{
		...
		$whoposted .= eval($templates->render("misc_whoposted_poster"));
		$altbg = alt_trow();
	}
	$numposts = my_number_format($numposts);
	$poster['posts'] = my_number_format($poster['posts']);
	$whop = eval($templates->render("misc_whoposted", 1, 0));
	echo $whop;
	exit;
}
elseif($mybb->input['action'] == "smilies")
{
	...
	{ // make small popup list of smilies
		...
		if(is_array($smilies_cache))
		{
			...
			foreach($smilies_cache as $smilie)
			{
				...
				$smilie_image = eval($templates->render("smilie", 1, 0));
				$smilies .= eval($templates->render("misc_smilies_popup_smilie"));
				if($e == 2)
				{
					$smilies .= "</tr><tr>";
					$e = 1;
		...
		if($e == 2)
		{
			$smilies .= eval($templates->render("misc_smilies_popup_empty"));
		}
		$smiliespage = eval($templates->render("misc_smilies_popup", 1, 0));
	...
	else
	{
		...
		if(is_array($smilies_cache))
		{
			...
			{
				$smilie['find'] = htmlspecialchars_uni($smilie['find']);
				$smilie_image = eval($templates->render("smilie"));
				$smilies .= eval($templates->render("misc_smilies_smilie"));
				$class = alt_trow();
			}
		}
		$smiliespage = eval($templates->render("misc_smilies"));
		output_page($smiliespage);
	}
}
elseif($mybb->input['action'] == "imcenter")
{
	...
	if($user['aim'])
	{
		...
		$navigationbar .= eval($templates->render("misc_imcenter_nav"));
		$navsep = ' - ';
	}
	if($user['skype'])
	{
	...
	$imcenter = eval($templates->render($imtemplate, 1, 0));
	echo $imcenter;
	exit;
}
elseif($mybb->input['action'] == "syndication")
{
	...
	if(is_array($forums))
	{
		...
		$feedurl = eval($templates->render("misc_syndication_feedurl"));
	}
	...
	$syndication = eval($templates->render("misc_syndication"));
...
function makesyndicateforums($pid="0", $selitem="", $addselect="1", $depth="", $permissions="")
{
	...
	if($addselect)
	{
		...
		$forumlist = eval($templates->render("misc_syndication_forumlist"));
	}
