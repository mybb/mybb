...
if($mybb->usergroup['issupermod'] != 1)
{
	...
	foreach($moderated_forums as $moderated_forum)
	{
		...
		if(is_moderator($moderated_forum, 'canviewdeleted'))
		{
...
if($mybb->input['action'] == "do_modqueue")
{
	...
	elseif(!empty($mybb->input['deletedthreads']))
	{
		...
		while($thread = $db->fetch_array($query))
		{
			...
			if($action == "restore" && ($mybb->usergroup['issupermod'] == 1 || (is_moderator($thread['fid'], 'canviewdeleted') && is_moderator($thread['fid'], 'canrestorethreads'))))
			{
				$threads_to_restore[] = $thread['tid'];
			}
			elseif($action == "delete" && ($mybb->usergroup['issupermod'] == 1 || (is_moderator($thread['fid'], 'canviewdeleted') && is_moderator($thread['fid'], 'candeletethreads'))))
			{
				$threads_to_delete[] = $thread['tid'];
			}
		}
...
if($mybb->input['action'] == "modqueue")
{
	...
	if($mybb->input['type'] == "deletedthreads" || (!$mybb->input['type'] && !$postqueue && !$threadqueue && !$attachmentqueue && ($counters['modqueue']['deletedthreads'] > 0 || $mybb->usergroup['issupermod'] == 1)))
	{
		...
		while($thread = $db->fetch_array($query))
		{
			...
			$thread['threadlink'] = get_thread_link($thread['tid']);
			$thread['forum_link'] = get_forum_link($thread['fid']);
			$thread['forum_name'] = htmlspecialchars_uni($forum_cache[$thread['fid']]['name']);
			$thread['threaddate'] = my_date('relative', $thread['dateline']);
			...
			if($thread['username'] == "")
			{
				if($thread['threadusername'] != "")
				{
					$thread['profile_link'] = htmlspecialchars_uni($thread['threadusername']);
				}
				else
				{
					$thread['profile_link'] = $lang->guest;
			...
			{
				$thread['profile_link'] = build_profile_link(htmlspecialchars_uni($thread['username']), $thread['uid']);
			}
	...
	if($mybb->input['type'] == "deletedposts" || (!$mybb->input['type'] && !$postqueue && !$threadqueue && !$attachmentqueue && !$deletedthreadqueue && ($counters['modqueue']['deletedposts'] > 0 || $mybb->usergroup['issupermod'] == 1)))
	{
		...
		while($post = $db->fetch_array($query))
		{
			...
			$post['postlink'] = get_post_link($post['pid'], $post['tid']);
			$post['forum_link'] = get_forum_link($post['fid']);
			$post['forum_name'] = htmlspecialchars_uni($forum_cache[$post['fid']]['name']);
			$post['postdate'] = my_date('relative', $post['dateline']);
			...
			if($post['username'] == "")
			{
				if($post['postusername'] != "")
				{
					$post['profile_link'] = htmlspecialchars_uni($post['postusername']);
				}
				else
				{
					$post['profile_link'] = $lang->guest;
			...
			{
				$post['profile_link'] = build_profile_link(htmlspecialchars_uni($post['username']), $post['uid']);
			}
