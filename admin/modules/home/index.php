...
elseif(!$mybb->input['action'])
{
	...
	$query = $db->simple_select("reportedcontent", "COUNT(*) AS repored_posts", "type = 'post' OR type = ''");
	$repored_posts = my_number_format($db->fetch_field($query, "repored_posts"));
	...
	$table->construct_cell("<strong>{$posts}</strong> {$lang->posts}<br /><strong>{$newposts}</strong> {$lang->new_today}<br /><a href=\"index.php?module=forum-moderation_queue&amp;type=posts\"><strong>{$unapproved_posts}</strong> {$lang->unapproved}</a><br /><strong>{$repored_posts}</strong> {$lang->repored_posts}<br /><strong>{$new_reported_posts}</strong> {$lang->unread_reports}", array('width' => '25%'));
	$table->construct_row();
