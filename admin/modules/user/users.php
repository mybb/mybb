...
if($mybb->input['action'] == "add")
{
	...
	output_custom_profile_fields($profile_fields['contact_required'], $mybb->get_input('profile_fields', MyBB::INPUT_ARRAY), $form_container, $form);
...
EOF;
	...
	$other_options = array(
		...
		"<label for=\"sourceeditor\">{$lang->editor_mode}:</label><br />".$form->generate_select_box("sourceeditor", array("0" => $lang->use_default, "1" => $lang->editor_mode_source, "2" => $lang->editor_mode_wysiwyg), $mybb->input['sourceeditor'], array('id' => 'sourceeditor')),
...
if($mybb->input['action'] == "delete")
{
	...
	{
		flash_message($lang->error_cannot_delete_self, 'error');
		admin_redirect("index.php?module=user-users");
	}
...
if($mybb->input['action'] == "merge")
{
	...
	if($mybb->request_method == "post")
	{
		...
		if(empty($errors))
		{
			...
			$db->update_query("securitylog", $uid_update, "uid='{$source_user['uid']}'");
...
function user_search_conditions($input, &$form)
{
	...
	$array = array('username', 'email', 'usergroup', 'website', 'website_blank', 'signature', 'signature_blank', 'usertitle', 'usertitle_blank', 'postnum_dir', 'postnum', 'threadnum_dir', 'threadnum', 'regdate', 'regip', 'lastip', 'postip');
