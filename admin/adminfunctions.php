...
// The code sucks right now. It could be improved alot :P
//////////
function add_menu_item($menuid, $item="", $position="end", $newmenu)
{
	global $menu;
	
	// Make sure the menu variable has been initialized first and is an array
	if(!$menu || !is_array($menu))
	{
		// If not, then return false
		// You need to make sure you call $plugins->add_hook("admin_index_navigation"); first
		return false;
	}
	
	// get the current menu array's id
	$menuitem = $menu[$menuid];
	// are we looking for an item in the menu array?
	if($item != "")
	{	
		// if so, then fetch it!
		$navitem = $menuitem['items'][$item];
		// Do we want to add a nav item to the beggining of the menu?
		if($position == "first")
		{
			// If so, then add it to the beggining!
			array_merge((array)$newmenu['item'], $navitems);
		}
		// Do we want to add it to the end?
		elseif($position == "end")
		{
			// If so, then add it!
			array_push($navitems, $newmenu['item']);
		}
		// If its not added to the beggining or end, then are we adding it to a specific point?
		else if(is_numeric($position))
		{
			// if that position doesn't already exist...
	...
	// So you left the menu item blank?
	else
	{
		// You must be adding a new menu then!
		if($position == "newmenu")
		{
		...
		// Otherwise I don't know what the hell you mean!
		else
		{
			return false;
		}
